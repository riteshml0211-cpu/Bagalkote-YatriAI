      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1">
            <div className="w-10 h-10 rounded-xl overflow-hidden bg-stone-800 shrink-0 border border-amber-500/40">
              <img
                src={monument.image}
                alt={monument.name}
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-amber-400 font-bold uppercase tracking-wider truncate">
                  {t.audioPlayer.title}
                </span>
              ...
              <h4 className="font-serif font-bold text-xs sm:text-sm text-white truncate max-w-[220px] sm:max-w-md">
                {currentTitle}
              </h4>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-3 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-stone-800 hover:bg-stone-700 text-amber-300 text-xs font-semibold border border-amber-500/30 transition-colors cursor-pointer shrink-0"
              title={language === 'en' ? 'Switch to Kannada audio' : 'Switch to English audio'}
            >
              <Globe className="w-3.5 h-3.5 shrink-0" />
              <span className="whitespace-nowrap">{language === 'en' ? t.audioPlayer.listenKn : t.audioPlayer.listenEn}</span>
            </button>

            {/* Playback speed */}
            <button
              onClick={toggleSpeed}
              className="px-2 py-1 rounded-lg bg-stone-800 hover:bg-stone-700 text-xs font-bold text-slate-300 transition-colors cursor-pointer shrink-0"
            ...
            <button
              ...
              disabled={isLoading}
              className="w-10 h-10 rounded-full bg-amber-500 hover:bg-amber-400 text-slate-950 flex items-center justify-center shadow-lg transition-transform active:scale-95 cursor-pointer disabled:opacity-75 shrink-0 ring-2 ring-amber-400/40"
            ...
            <button
              ...
              className="text-xs text-slate-300 hover:text-white flex items-center gap-1 cursor-pointer shrink-0 px-1 py-1"
            >
